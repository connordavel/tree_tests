...
unsigned int sizebuf; //  Color buffer
...
//  Reset velocities
   ...
   for (int i=0;i<n;i++ )
   {
      vel[i].x = frand(-0.1,+0.1);
      vel[i].y = frand(-0.1,+0.1);
      vel[i].z = frand(-0.1,+0.1);
      vel[i].w = 0;
   }
   glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
   ...
   glBindBufferBase(GL_SHADER_STORAGE_BUFFER,6,sizebuf);
...
void render_boids() {
   ...
   // //  Color array
   // glBindBuffer(GL_ARRAY_BUFFER,sizebuf);
   ...
   glBindBuffer(GL_ARRAY_BUFFER,posbuf);
   vec4 pos_data[n];
   glGetBufferSubData(GL_ARRAY_BUFFER, 0, n*sizeof(vec4), pos_data);
   glBindBuffer(GL_ARRAY_BUFFER,sizebuf);
   vec4 size_data[n];
   glGetBufferSubData(GL_ARRAY_BUFFER, 0, n*sizeof(vec4), size_data);
   for (int i=0; i<n; i++){
      vec4 t = pos_data[i];
      vec4 s = size_data[i];
...
int main(int argc,char* argv[])
{
   ...
   //  Create the window
   glutCreateWindow("Final Project (Boids), Connor Davel");
